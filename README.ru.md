...
- 🔎 **Безопасный preview** — `/quote` показывает текущего лидера без публикации и очистки данных
...
| :--- | :--- | :--- |
| **Реакции** | 20% | Нормализованное количество эмодзи-реакций |
...
| `/quote` | Безопасный preview текущего открытого окна |
...
`/quote` теперь только показывает preview. `/publish_quote` нужен администраторам, если автоматический запуск пометил окно как скучное или упал с ошибкой.
