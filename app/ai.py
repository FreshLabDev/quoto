...
_IMAGE_DESCRIPTION_PROMPT = (
    "Это статичное изображение. Опиши только то, что реально видно на изображении: объекты, людей, "
    "действия/позы, обстановку, стиль, эмоции и заметный текст/надписи. Не выдумывай звук, движение, "
    "события до/после кадра или скрытый контекст. Пиши одним цельным кратким, но достаточно полным "
    "описанием. Ответ должен состоять только из описания."
)

_STICKER_DESCRIPTION_PROMPT = (
    "Это Telegram-стикер или мем-картинка. Опиши только то, что реально видно: персонажа/объект, "
    "мимику, позу, стиль, надписи и общий мемный смысл. Не выдумывай звук, движение или внешний "
    "контекст. Пиши одним цельным кратким описанием. Ответ должен состоять только из описания."
)

_VIDEO_DESCRIPTION_PROMPT = (
    "Это видео, анимация или видеокружок. Опиши только то, что реально видно и слышно в медиа: "
    "последовательность событий, людей/объекты, действия, обстановку, заметный текст, речь и звуки. "
    "Если речь или звук неразборчивы, прямо напиши, что они неразборчивы. Не выдумывай причины, "
    "имена, место или события вне кадра. Пиши одним цельным кратким, но достаточно полным описанием. "
    "Ответ должен состоять только из описания."
)

_AUDIO_DESCRIPTION_PROMPT = (
    "Это только аудио без изображения. Описывай исключительно то, что слышно. Запрещено описывать "
    "внешность, камеру, одежду, помещение, улицу, предметы, действия в кадре или видеоряд, если это "
    "не произнесено прямо в аудио. Передай речь как краткий транскрипт или пересказ с точными "
    "ключевыми фразами в кавычках, затем укажи тон, эмоции, паузы, смех, шумы и фоновые звуки, если "
    "они слышны. Если речь неразборчива, так и напиши. Не делай выводов о личности говорящего или "
    "месте записи сверх услышанного. Ответ должен состоять только из описания слышимого."
)
...
def _media_description_prompt(media_kind: str) -> str:
    if media_kind in {"voice", "audio"}:
        return _AUDIO_DESCRIPTION_PROMPT
    if media_kind in {"video", "animation", "video_note"}:
        return _VIDEO_DESCRIPTION_PROMPT
    if media_kind == "sticker":
        return _STICKER_DESCRIPTION_PROMPT
    return _IMAGE_DESCRIPTION_PROMPT
